...
const EarringsDesc = ({ addToCart }) => {
  const { id } = useParams();
  const [index, setIndex] = useState(0);
  const myRef = useRef();

  // Find the bangle with the matching id
  const earrings = EarringsData.find((b) => b.id === parseInt(id));
  ...
  return (
    <div className="app">
      <div>
        <div className="details" key={earrings.id}>
          <div className="big-img">
            <img src={earrings.Img[index]} alt="" />
          </div>

          <div className="box">
            <div className="row">
              <h4 className="header">{earrings.Title}</h4>
              <h5>{earrings.Cat}</h5>
            </div>
            <div>
              <p className="price">{earrings.Price}</p>
            </div>
            ...
            <Link to='/Cart'>
              <button className="cart" onClick={() => addToCart(earrings)}>Add to cart</button></Link>
            <button className="cart">Buy</button>
          </div>
        </div>
        <div className="product-details-container">
          <table className="product-details-table fs-4">
            {earrings.Details.map((category, categoryIndex) => (
