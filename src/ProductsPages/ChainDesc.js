...
const ChainDesc = ({addToCart}) => {
  const { id } = useParams();
  const [index, setIndex] = useState(0);
  const myRef = useRef();

  // Find the bangle with the matching id
  const chain = ChainsData.find((b) => b.id === parseInt(id));
  ...
  return (
    <div className="app">
      <div>
        <div className="details" key={chain.id}>
          <div className="big-img">
            <img src={chain.Img[index]} alt="" />
          </div>

          <div className="box">
            <div className="row">
              <h4 className="header">{chain.Title}</h4>
              <h5>{chain.Cat}</h5>
            </div>
            <div>
            ...
            <Details images={chain.Img} tab={handleTab} myRef={myRef} />
            ...
            <Link to='/Cart'>
            <button className="cart" onClick={() =>addToCart(chain)} >Add to cart</button></Link>
            <button className="cart">Buy</button>
          </div>
        </div>
        <div className="product-details-container">
        <table className="product-details-table fs-4">
  {chain.Details.map((category, categoryIndex) => (
